...
const char *get_used_functions(const char *test_name)
{
  if (strcmp(test_name, "test_init") == 0)
    return "new_rbtree, delete_rbtree";
  if (strcmp(test_name, "test_insert_single") == 0)
    return "new_rbtree, rbtree_insert, delete_rbtree";
  if (strcmp(test_name, "test_find_single") == 0)
    return "new_rbtree, rbtree_insert, rbtree_find, delete_rbtree";
  if (strcmp(test_name, "test_erase_root") == 0)
    return "new_rbtree, rbtree_insert, rbtree_erase, delete_rbtree";
  if (strcmp(test_name, "test_minmax_suite") == 0)
    return "new_rbtree, rbtree_insert, rbtree_min, rbtree_max, rbtree_erase, delete_rbtree";
  if (strcmp(test_name, "test_to_array_suite") == 0)
    return "new_rbtree, rbtree_insert, rbtree_to_array, delete_rbtree";
  if (strcmp(test_name, "test_distinct_values") == 0)
    return "new_rbtree, rbtree_insert, delete_rbtree (그리고 RB-Tree 규칙 검사)";
  if (strcmp(test_name, "test_duplicate_values") == 0)
    return "new_rbtree, rbtree_insert, delete_rbtree (그리고 RB-Tree 규칙 검사)";
  if (strcmp(test_name, "test_multi_instance") == 0)
    return "new_rbtree, rbtree_insert, rbtree_to_array, delete_rbtree";
  if (strcmp(test_name, "test_find_erase_fixed") == 0)
    return "new_rbtree, rbtree_insert, rbtree_find, rbtree_erase, delete_rbtree";
  if (strcmp(test_name, "test_find_erase_rand") == 0)
    return "new_rbtree, rbtree_insert, rbtree_find, rbtree_erase, delete_rbtree";
  return "알 수 없음";
}
...
{
  if (strcmp(test_name, "test_init") == 0)
    return "새로운 RB-Tree가 올바르게 초기화되는지 확인해요! (tree->root == tree->nil)";
  if (strcmp(test_name, "test_insert_single") == 0)
    return "노드 한 개를 삽입했을 때, root가 잘 설정되는지 확인해요!";
  if (strcmp(test_name, "test_find_single") == 0)
    return "노드 한 개를 넣고, 잘 찾아지는지 확인해요!";
  if (strcmp(test_name, "test_erase_root") == 0)
    return "단 하나뿐인 루트 노드를 지웠을 때, 트리가 비어있는 상태가 되는지 확인해요!";
  if (strcmp(test_name, "test_find_erase_fixed") == 0)
    return "미리 정해진 값들을 넣고, 순서대로 잘 지워지는지 확인해요!";
  if (strcmp(test_name, "test_minmax_suite") == 0)
    return "여러 값을 넣었을 때, 최솟값과 최댓값을 잘 찾는지 확인해요!";
  if (strcmp(test_name, "test_to_array_suite") == 0)
    return "트리의 모든 값을 배열로 잘 변환하는지 확인해요!";
  if (strcmp(test_name, "test_distinct_values") == 0)
    return "중복 없는 값들을 넣었을 때, RB-Tree의 규칙(색깔, 순서)을 잘 지키는지 확인해요!";
  if (strcmp(test_name, "test_duplicate_values") == 0)
    return "중복 있는 값들을 넣었을 때, RB-Tree의 규칙(색깔, 순서)을 잘 지키는지 확인해요!";
  if (strcmp(test_name, "test_multi_instance") == 0)
    return "여러 개의 트리를 동시에 만들어도 서로 영향을 주지 않는지 확인해요!";
  if (strcmp(test_name, "test_find_erase_rand") == 0)
    return "랜덤한 값들을 넣고 지웠을 때도, 문제가 없는지 확인해요!";
  return "설명이 없는 테스트에요...";
}
